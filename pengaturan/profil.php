...
{
?>
	<body onLoad="document.form.elements['nama_sekolah'].focus();">
	<div class="post">
		<div class="entry">
		    <h2 align="center"><strong>Profil</strong></h2>
		    <p align="center">&nbsp;</p>
		    <p>
			<form action="?page=./pengaturan/profil" method="post" name="form">
			<table>
			<tr><td>Nama Sekolah</td><td><input type="text" name="nama_sekolah" size="15"/></td></tr>
			...
			while($row=mysql_fetch_array($query)){
				?>
				<tr><td>Nama Sekolah</td><td>:</td><td><?php echo $row['nama_sekolah'];?></td></tr>
			...
			if(isset($_POST['nama_sekolah'])){
				...
				$query=mysql_query("insert into tabel_profil 
									values('$nama_sekolah','$gedung','$jalan','$kelurahan','$kecamatan','$propinsi','$negara','$telpon','$fax','$email','$website')");
				
				if($query){
					?><script language="javascript">document.location.href="?page=./pengaturan/profil"</script><?php
				}else{
					echo mysql_error();
				}
